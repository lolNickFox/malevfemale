...
female_words10 <- readr::read_lines("female/Bonetta 2006 Aftermath of scientific fraud.txt")
...
words_male <- dplyr::bind_rows(mwords1, mwords2, mwords3, mwords4, mwords5, mwords6, mwords7, mwords8, mwords9, mwords10) %>% 
  mutate(gender = "male")

words_female <- dplyr::bind_rows(fwords1, fwords2, fwords3, fwords4, fwords5, fwords6, fwords7, fwords8, fwords9, fwords10) %>% 
...
word_sentiment %>%
  ...
  ggplot(aes(x = gender, y = mean_sentiment)) + geom_col()
...
test %>% 
  ggplot(aes(word2, logratio, color = logratio < 0)) + geom_segment(aes(x = word2, xend = word2, y = 0, yend = logratio), size = 1.1, alpha = 0.6) + coord_flip () + geom_point(size = 3.5) + labs(x = NULL, 
         y = "log odds ratio (female / male)",
         title = "Words appearing in female or male first-authored papers") +
    scale_color_discrete(name = "", labels = c("Female first author", "Male first author")) + scale_y_continuous(breaks = seq(-7, 7),
                       labels = c("0.008x", "0.015625x", "0.03125x", "0.0625x", "0.125x", "0.25x", "0.5x", 
                                  "Same", "2x", "4x", "8x", "16x", "32x", "64x", "128x"))
